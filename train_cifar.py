...
parser.add_argument('--learning_rate', type=float, default=0.013, help='init learning rate')
...
parser.add_argument('--arch', type=str, default='PDARTS_V3', help='which architecture to use')
parser.add_argument('--grad_clip', type=float, default=5, help='gradient clipping')
parser.add_argument('--tmp_data_dir', type=str, default='/home/grouplcheng/data/zch/dataset/cifar10/train', help='temp data dir')
parser.add_argument('--note', type=str, default='try', help='note for this run')
parser.add_argument('--cifar100', action='store_true', default=False, help='if use cifar100')

args, unparsed = parser.parse_known_args()

args.save = 'save_path{}eval-{}-{}'.format(args.save, args.note, time.strftime("%Y%m%d-%H%M%S"))
...
def main():
    ...
    writerTf = SummaryWriter('argument_cifar')
    ...
    valid_queue = torch.utils.data.DataLoader(
        train_data, batch_size=args.batch_size, sampler=torch.utils.data.sampler.SubsetRandomSampler(indices[split:num_train]), pin_memory=True, num_workers=args.workers)
