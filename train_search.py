...
parser.add_argument('--learning_rate', type=float, default=0.017, help='init learning rate')
parser.add_argument('--learning_rate_min', type=float, default=0.00009, help='min learning rate')
...
parser.add_argument('--tmp_data_dir', type=str, default='/home/grouplcheng/data/zch/dataset/cifar10/train',
                    help='temp dataset dir path')
...
args.save = 'save_path{}search-{}-{}'.format(args.save, args.note, time.strftime("%Y%m%d-%H%M%S"))
        ...
        for epoch in range(epochs):
            ...
            if epoch < eps_no_arch:
                ...
                train_acc, train_obj = train(train_queue, valid_queue, model, network_params, criterion, optimizer,
                                             optimizer_a, lr, train_arch=False, temperature=temperature)
            else:
                model.module.p = float(drop_rate[sp]) * np.exp(-(epoch - eps_no_arch) * scale_factor)
                model.module.update_p()
                train_acc, train_obj = train(train_queue, valid_queue, model, network_params, criterion, optimizer,
                                             optimizer_a, lr, train_arch=True, temperature=temperature)
            ...
            if epochs - epoch < 5:
                valid_acc, valid_obj = infer(valid_queue, model, criterion, temperature=temperature)
                logging.info('Valid_acc %f', valid_acc)
                writerTf.add_scalar(sp_str + 'valid_acc', valid_acc, epoch)
                writerTf.add_scalar(sp_str + 'valid_obj', valid_obj, epoch)

            writerTf.add_scalar(sp_str + 'train_acc', train_acc, epoch)
            writerTf.add_scalar(sp_str + 'train_obj', train_obj, epoch)
            writerTf.close()
...
def train(train_queue, valid_queue, model, network_params, criterion, optimizer, optimizer_a, lr, train_arch=True,
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        if train_arch:
            ...
            logits = model(input_search, temperature)
...
def infer(valid_queue, model, criterion, temperature=5.0):
    objs = utils.AvgrageMeter()
    top1 = utils.AvgrageMeter()
    top5 = utils.AvgrageMeter()
    model.eval()
